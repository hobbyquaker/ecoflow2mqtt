...
export const RuntimePropertyUpload = root.lookupType('RuntimePropertyUpload');
...
export const MESSAGES = {
    '254/21': {name: 'DisplayPropertyUpload', type: DisplayPropertyUpload, items: true},
    '254/22': {name: 'RuntimePropertyUpload', type: RuntimePropertyUpload},
};
 ...
 *   items: boolean, error: string|null}>}
 */
export function decodeFrames(payload) {
    ...
    return (message.header ?? []).map((header) => {
        ...
        try {
            ...
            frame.items = Boolean(known.items);
        } catch (error) {
