...
# ecoflow2mqtt@<name> services there. Same script as in lgtv2mqtt / wiim2mqtt / cul2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/ecoflow2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                ecoflow2mqtt@<name> instance plus a plain ecoflow2mqtt unit if present
...
#   sudo ecoflow2mqtt --install -n ecoflow -u mqtt://broker \
#       --email you@example.com --password '...' --sn BK01Z...
# Better: pipe the credentials in instead of putting them on the command line, e.g.
#   ssh host 'bash -s' <<< 'ECOFLOW2MQTT_EMAIL=... ECOFLOW2MQTT_PASSWORD=... ecoflow2mqtt --install ...'
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n ecoflow -u mqtt://broker --email ... --password ... --sn ...)"
fi
